...
class TestHomePageView:
    ...
    def setup_data(self):
        ...
        reviews = ReviewFactory.create_batch(2)

        # Create sample recommended animes
        recommended_animes = AnimeFactory.create_batch(
            2,
    ...
    def test_generates_and_caches_data(self, anonymous_user, setup_data):
        ...
        # Verify data lengths
        assert len(data["current_season"]) == 2
        assert len(data["anime_reviews"]) == 2
        assert len(data["anime_recommendations"]) == 2
